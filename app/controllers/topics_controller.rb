class TopicsController < ApplicationController
  def index
    @topics = Topic.public.with_read_marks_for(current_user).page(params[:page]).order('last_activity_at DESC').includes(:tags)
  end
  ...
  def search
    ...
    @topics        = Topic.public.with_read_marks_for(current_user).with_tags(@searched_tags).page(params[:page]).order('last_activity_at DESC').includes(:tags)
  end


  def new
    authorize! :create, Topic
    @topic = Topic.public.new
  end
  ...
  def show
    @topic   = Topic.public.with_score_values_for(current_user).find(params[:id])
  ...
  def edit
    @topic = Topic.public.find(params[:id])
